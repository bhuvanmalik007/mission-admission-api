  ...
  request({
    ...
    .then(response => {
      ...
      if (i[0].subsenses) {
        ...
        res.send({words: [j, ...k], pronounciation: JSON.parse(response.body).results[0].lexicalEntries[0].pronunciations[0].audioFile});
      } else {
        let j = i.map(obj => ({ word: req.params.word, meaning: obj.definitions[0], example: obj.examples && obj.examples[0].text }));
        res.send({words: j, pronounciation: JSON.parse(response.body).results[0].lexicalEntries[0].pronunciations[0].audioFile});
      }
    }, err => {
      console.log(err);
      res.send({words:[]});
